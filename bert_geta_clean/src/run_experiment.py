"""
BERT-base × GETA full SQuAD experiment driver.

Runs joint pruning + quantization on bert-base-uncased over full SQuAD v1.1,
then evaluates the compressed subnet.

Layout assumed:
  bert_geta_clean/
  ├── geta/                  <- GETA / OTO upstream clone (with patches 01,02)
  ├── src/run_experiment.py  <- this file
  └── src/quant_fix.py       <- runtime monkey-patch (fix #3)

The two upstream patches must already be applied. See
docs/BERT_GETA_PATCH_GUIDE.md.

Usage:
  python run_experiment.py --sparsity 0.7 --epochs 13 --batch_size 16 \\
      --lr 6e-5 --start_pruning_epoch 5 --pruning_end_epoch 10 \\
      --bit_reduction 1 --lr_scheduler linear
"""
import argparse
import collections
import json
...
# TF32 + cudnn autotuner (Ampere+ GPUs, ~1.3-1.5x speedup)
...
def parse_args():
    p = argparse.ArgumentParser(description="BERT × GETA SQuAD experiment")
    p.add_argument("--sparsity", type=float, nargs="+", required=True,
                    help="target group sparsity, one or more (e.g. 0.1 0.3 0.5 0.7)")
    ...
    p.add_argument("--start_pruning_epoch", type=float, default=5.0,
                    help="epoch at which pruning starts (paper=5)")
    p.add_argument("--pruning_end_epoch", type=float, default=10.0,
                    help="epoch at which pruning finishes (paper=10)")
    p.add_argument("--lr_scheduler", type=str, default="linear", choices=["none", "linear"],
                    help="LR schedule for both lr and lr_quant (paper=linear: warmup+decay)")
...
def run_single(sparsity, args, tokenizer, raw_val, train_ds, val_ds, log):
    ...
    # Fix #4: explicitly compute projection_steps so it isn't 1, which would
    # cause `1 // projection_periods = 0` and a divide-by-zero in step().
...
def main():
    ...
    log_path = os.path.join(args.out_root, f"run_{ts}.log")
    ...
    log.info(f"\n[OK] Run complete. Results in {args.out_root}/")
