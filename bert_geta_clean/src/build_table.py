"""
Build a paper-Table-3-style summary from training results.

Loads each sp*/result.json under --results_dir, computes a baseline BOPs from
a fresh uncompressed BERT-base QA (run through the same model_to_quantize_model
+ OTO pipeline used during training), then prints:

    Target | Actual Sparsity | EM(%) | F1(%) | BOPs(G) | Rel.BOPs(%)

Run after training finishes. Read-only — does not affect training.

Usage:
    python build_table.py
    python build_table.py --results_dir ./results
    python build_table.py --baseline_bops_m 12345.6   # skip baseline computation
    python build_table.py --out table.md              # also write markdown table
"""
import argparse
import glob
import json
