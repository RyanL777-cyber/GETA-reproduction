...
- ✅ M1 load BERT
- ✅ M2 quant wrap（73 層）
- ✅ M3 OTO graph（1102 nodes, 1177 edges）
- ✅ M3.5 param dedup check（model 自身無重複）
- ✅ M4 `oto.geta(...)` 建 optimizer
- ✅ M5 2 batch forward/backward/step（loss 5.9593 → 5.9469，符合隨機 QA head 的理論值 log(384)≈5.95）

**介面打通完成**。剩下的風險列在 `geta_knowledge/open_questions.md` Q9，進 Phase 4 前要留意。

---

## 9. 未確認的事項（Phase 4 要盯）

1. **修完 dedup 後 27 groups 是否都保留**（預期是 27，沒有空組被刪）。
2. **修完 dedup 後 smoke.py 是否還通**（M4/M5 應該照樣過）。
...
| `bug_analysis/dump_duplicate_params.py` | 一次性診斷 script，列出 GETA 把哪些 param 塞進多個 group |
| `bug_analysis/stdout` | dump script 完整輸出，保存證據 |
| `bug_analysis/BUG_ANALYSIS.md` | 初版 workaround 的分析（server copilot 寫的，邏輯只覆蓋到「為什麼要 dedup」） |
